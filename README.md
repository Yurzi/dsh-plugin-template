...
A production-oriented template for third-party [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) plugins. It mirrors current internal plugin contracts while remaining installable outside the DSH monorepo.

## Included

- Named Cordis exports: `name`, `inject`, `Config`, and `apply`.
- One strict model tool with an explicit output schema and renderer.
...
3. Client dependencies must be DSH browser module-table entries or bundled; never use Node APIs in `src/client`.
4. Every object in a model-facing JSON schema sets `additionalProperties: false`.
5. Keep `dsh.client.inject`, client `inject`, imports, and slot ownership aligned.
6. Treat `cordis.patch.yml` values as first-boot composition defaults.
